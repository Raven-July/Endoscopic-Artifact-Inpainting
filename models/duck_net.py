...
class Normalization(nn.Module):
    def __init__(self, num_features, normalization="batch"):
        ...
        if normalization is None:
            ...
            raise ValueError(
                "Invalid normalization type. Supported types are None, 'batch', and 'instance'."
            )
...
class SeparatedConv2dBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, normalization="batch"):
        ...
        self.block = nn.Sequential(
            nn.Conv2d(
                in_channels, out_channels, (1, kernel_size), stride=1, padding="same"
            ),
            nn.ReLU(),
...
class DuckConv2dBlock(nn.Module):
    def __init__(
        ...
        self.wide_scope = WideScopeConv2dBlock(
            in_channels, out_channels, kernel_size, normalization
        )
        self.mid_scope = MidScopeConv2dBlock(
            in_channels, out_channels, kernel_size, normalization
        )
        self.residual_block_1 = ResnetConv2dBlock(
            in_channels,
            out_channels,
            kernel_size,
            normalization,
        ...
        self.separated_block = SeparatedConv2dBlock(
            in_channels, out_channels, kernel_size, normalization
        )
...
class DuckNet(nn.Module):
    def __init__(
        ...
        residual_block_multiplier = (
            1.0  # multiplier for the residual block's addition operation
        )
        self.addition_multiplier = (
            1.0  # multiplier in the encoder-decoder skip connection
        )
        ...
        self.multi_scale_blocks = nn.ModuleList(
            [
                nn.Conv2d(
                    in_channels, init_features * 2, kernel_size=2, stride=2, padding=0
                )
            ]
        )
        for d in range(1, depth):
            self.multi_scale_blocks.append(
                nn.Conv2d(
                    init_features * (2**d),
                    init_features * (2 ** (d + 1)),
                    kernel_size=2,
        ...
        self.down_blocks = nn.ModuleList(
            [
                nn.Conv2d(
                    init_features, init_features * 2, kernel_size=2, stride=2, padding=0
                )
            ]
        )
        for d in range(1, depth):
        ...
        # encoder duck blocks
        self.encoder_duck_blocks = nn.ModuleList(
            [
                DuckConv2dBlock(
                    in_channels,
                    init_features,
                    ...
                    residual_block_multiplier=residual_block_multiplier,
                )
            ]
        )
        for d in range(1, depth):
            self.encoder_duck_blocks.append(
                DuckConv2dBlock(
                    init_features * (2**d),
                    init_features * (2**d),
                    normalization=normalization,
        ...
        self.bottleneck = nn.Sequential(
            ResnetConv2dBlock(
                ...
                kernel_size=3,
                normalization=normalization,
                multiplier=residual_block_multiplier,
            ),
            ResnetConv2dBlock(
                init_features * (2**depth),
                init_features * (2**depth),
        ...
        # decoder duck blocks
        self.decoder_duck_blocks = nn.ModuleList(
            [
                DuckConv2dBlock(
                    init_features,
                    init_features,
        ...
        for d in range(1, depth):
            self.decoder_duck_blocks.append(
                DuckConv2dBlock(
                    init_features * (2 ** (d)),
                    init_features * (2 ** (d - 1)),
                    normalization=normalization,
        ...
        self.out = nn.Conv2d(
            init_features, out_channels, kernel_size=1, stride=1, padding="same"
        )
        ...
        if out_activation is None:
            ...
            raise ValueError(
                "Invalid out_activation type. Supported types are 'sigmoid', 'softmax', 'relu', and None."
            )
